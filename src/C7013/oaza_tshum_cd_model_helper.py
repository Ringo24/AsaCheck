...
class OazaTshumCdModelHelper(object):
    '''
    大字通称コード変換用モデルを構築するためのHelperクラスです
    '''
    ...
    def assembly_model(self) -> None:
        ...
        dense_layer = layers.Dense(name='concatenated_dense', units=self._one_hot_encoder.oaza_tshum_cd_length*2, activation='relu')(dropout_layer)

        # 出力を定義
        oaza_tshum_cd_output_layer = layers.Dense(name='oaza_tshum_cd_output', units=self._one_hot_encoder.oaza_tshum_cd_length, activation='softmax')(dense_layer)

        # モデル定義とコンパイル
        self._model = models.Model(
            name='oaza_tshum_cd_classification_model',
            inputs={
            ...
            outputs={
                'oaza_tshum_cd_output': oaza_tshum_cd_output_layer,
            })
    ...
    def fit_model_from_file(self, train_file_path: pathlib.PurePath, val_file_path: pathlib.PurePath, batch_size: int, epochs: int, workers: int = 0) -> None:
        ...
        Args:
            ...
            workers:workers
        '''
        self.logger.info(f'大字通称コード分類モデルを訓練します batch_size={batch_size}, epochs={epochs}')
        train_gen = OazaTshumCdTrainingGenerator(self._one_hot_encoder, self._vectorizer, train_file_path, batch_size=batch_size)
        val_gen = OazaTshumCdTrainingGenerator(self._one_hot_encoder, self._vectorizer, val_file_path, batch_size=batch_size)
    ...
    def load_model(self, load_path: pathlib.PurePath = None) -> None:
        ...
        model_path = load_path / 'oaza_tshum_cd_model.h5'
        self.logger.info(f'大字通称コード変換用モデルを読み込みます load_path={model_path}')
    ...
    def save_model(self, save_path: pathlib.PurePath = None) -> None:
        ...
        model_path = save_path / 'oaza_tshum_cd_model.h5'
        self.logger.debug(f'大字通称コード変換用モデルを保存します save_path={model_path}')
        # include_optimizer=Falseの場合、モデルを再学習することはできないが、出力サイズを抑制できる
        self._model.save(model_path, include_optimizer=False)
        plot_model(self._model, to_file=save_path / 'oaza_tshum_cd_model.png', show_shapes=True, show_layer_names=True)
    ...
    def save_training_accuracy_and_loss(self) -> None:
        ...
        utils.save_training_accuracy_and_loss(self._training_history, 'oaza_tshum_cd_model_training_accuracy_and_loss.png')
    ...
    def save_training_and_validation_loss(self) -> None:
        ...
        utils.save_training_and_validation_loss(self._training_history, 'oaza_tshum_cd_model_training_and_validation_loss.png')
    ...
    def predict(
        ...
        Returns:
            大字通称コードのNumpy配列
        '''
        self.logger.debug('大字通称コードを予測します')
        ...
        predV = self._model.predict(
            ...
            batch_size=batch_size,
            verbose=verbose)
        return self._one_hot_encoder.oaza_tshum_cd_inverse_transform(predV['oaza_tshum_cd_output'])
    ...
    def evaluate(
        ...
        Args:
            ...
            scyosn_cd_list: 市区町村コードのNumpy２次元配列
            addr_nm_list: 住所のNumpy配列
            oaza_tshum_cd_list: 大字通称コードのNumpy配列
            batch_size: バッチサイズ
        ...
        test_loss, test_acc = self._model.evaluate(
            ...
            y={
                'oaza_tshum_cd_output': oaza_tshum_cdV
            },
            batch_size=batch_size,
            verbose=verbose
        )
...
class OazaTshumCdTrainingGenerator(Sequence):
    '''
    大字通称コード training generatorクラスV2
    '''
    ...
    def __init__(self, one_hot_encoder: AddresscodeOneHotEncoder, vectorizer: JapaneseSentenceVectorizer, file_path: pathlib.PurePath, batch_size: int = 1):
        ...
        self._input_data: pd.DataFrame = pd.read_csv(file_path, sep=',', encoding='utf-8', dtype=object, usecols=['addr_nm', 'tdfkn_cd', 'scyosn_cd', 'oaza_tshum_cd'])
        ...
        self._tdfkn_scyosn_cd_input = self._one_hot_encoder.tdfkn_scyosn_cd_transform(self._input_data[['tdfkn_cd', 'scyosn_cd']].values)
        self._oaza_tshum_cd_output = self._one_hot_encoder.oaza_tshum_cd_transform(self._input_data[['oaza_tshum_cd']].values)
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        data = ({
            ...
            'oaza_tshum_cd_output': self._oaza_tshum_cd_output[start_pos:end_pos],
            })
        return data

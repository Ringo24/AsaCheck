"""
05_市区町村コード予測.py
住所+都道府県コードを元に市区町村コードを予測して結果を保存する
"""
...
def main():
    ...
    #学習モデルで予測_検証データ
    pred = classifier.predict(address_list=data_val['住所'].values, tdfkn_cd_list=data_val['都道府県コード'].values, batch_size=2, verbose=1)

    data_val['市区町村コード予測'] = pred
    data_val["市区町村コード予測結果"] = data_val['市区町村コード'] == pred

    #Excelに結果を書き込む
    data_val.to_excel(pathlib.Path(__file__).parent / 'data/04_市区町村コード予測_検証データ.xlsx', index=False)

    #学習モデルの評価_検証データ
    print(classifier.metrics_names)
    print(classifier.evaluate_model(data_val['住所'].values, data_val['都道府県コード'].values, data_val['市区町村コード'].values, batch_size=2))
